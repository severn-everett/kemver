...
object XRangeProcessor : Processor {
    ...
    override fun process(range: String): String {
        val ranges =
            range.split(spaceRegex).mapNotNull { rangeVersion ->
                xRangeRegex.matchEntire(rangeVersion)?.let { matchResult ->
                    ...
                    val compareSign =
                        matchResult.groupValues[1].takeUnless { it == Range.RangeOperator.EQ.asString && patch.isX() }
                            ?: ""
